...
{
    public partial class fmLoaiSach : UserControl
    {
        public fmLoaiSach()
        {
            InitializeComponent();
        ...
        Int16 TrangThai = 0;
        Decimal Ma = 0;
        clLoaiSach clLoaiSach = new clLoaiSach();
        clLoaiSach ojp = new clLoaiSach();
        private void fmLoaiSach_Load(object sender, EventArgs e)
        {
            Ham.ThemLuocSu(Ham.MaNV, "Loại Sách", "Xem", "");
        ...
        private void bntLuu_Click(object sender, EventArgs e)
        {
            if (txtTen.Text == "")
            {
                Ham.KhungTB(3, "Tên loại sách không được rổng", pnThongBao, lbThongBao);//
                txtTen.Focus();
                return;
            }
            if (TrangThai == 1)
        ...
        private void gridView1_RowClick(object sender, DevExpress.XtraGrid.Views.Grid.RowClickEventArgs e)
        {
            ...
            bntXoa.Enabled = true;
            Ma = Convert.ToDecimal(a.Replace("LS00", ""));
            if (TrangThai == 2)
                this.LoadTxt();
        }
        private void LoadGV()
        {
            gridControl1.DataSource = clLoaiSach.LayDS();
            gridView1.GroupPanelText = "Tổng số dòng là: " + clLoaiSach.Dem();

        }
        private void LoadTxt()
        {
            ojp = clLoaiSach.LayDS_Ma(Ma);
            txtTen.Text = ojp.TenLS;
        ...
        private void Them()
        {
            ...
            clLoaiSach.TenLS = txtTen.Text;
            clLoaiSach.GhiChu = txtGhiChu.Text;
            clLoaiSach.TrangThai = cbTrangThai.Checked;
            if (clLoaiSach.Them(clLoaiSach) > 0)
            {
                Ham.KhungTB(1, "Thêm thành công", pnThongBao, lbThongBao);//
                Ham.ThemLuocSu(Ham.MaNV, "Thêm loại sách: " + txtTen.Text, "Thêm", "");
            }
            else
        ...
        private void Sua()
        {
            ...
            clLoaiSach.GhiChu = txtGhiChu.Text;
            clLoaiSach.TrangThai = cbTrangThai.Checked;
            if (clLoaiSach.Sua(clLoaiSach) == true)
            {
                Ham.KhungTB(1, "Thay đổi thành công", pnThongBao, lbThongBao);//
                Ham.ThemLuocSu(Ham.MaNV, "Thay đổi loại sách " + Ma + " thành: " + txtTen.Text, "Thay đổi", "");
            }
            else
